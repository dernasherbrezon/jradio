...
public class BpskDemodulator implements ByteInput {
	...
	public BpskDemodulator(FloatInput source, int symbolRate, int decimation, double centerFrequency, boolean differential) {
		...
		next = new LowPassFilterComplex(next, 1.0, symbolRate, symbolRate * 0.1, Window.WIN_HAMMING, 6.76);
		int nfilts = 16;
		...
		next = new CostasLoop(next, (float) (2 * Math.PI / symbolRate * 50), 2, false);
		if (differential) {
