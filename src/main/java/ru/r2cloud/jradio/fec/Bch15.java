...
public class Bch15 {
	...
	public static void decode(byte[] array, int offset, int d) throws UncorrectableException {
		...
		for (int i = 0; i < (d - 1); i++) {
			int cur = computeSyndrome(data, i);
			if (cur != 0) {
				allZero = false;
			}
			syndromes[i] = cur;
		...
		List<Integer> errorLocations = computeErrorLocations(syndromes);
		for (int i = 0; i < errorLocations.size(); i++) {
			array[offset + errorLocations.get(i)] ^= 1;
		}
	}

	private static List<Integer> computeErrorLocations(int... syndromes) throws UncorrectableException {
		...
		if (syndromes.length == 2) {
			...
			int detS = gfMultiply(syndromes[0], syndromes[2]) ^ gfMultiply(syndromes[1], syndromes[1]);
			if (detS == 0) {
				return computeErrorLocations(syndromes[0], syndromes[1]); // # matrix is non-invertible, throw away 2 syndromes
			}
			int invDetS = gfInvert(detS);
			l[2] = gfMultiply(syndromes[2], syndromes[2]) ^ gfMultiply(syndromes[3], syndromes[1]);
			l[2] = gfMultiply(l[2], invDetS);
			l[1] = gfMultiply(syndromes[0], syndromes[3]) ^ gfMultiply(syndromes[2], syndromes[1]);
			l[1] = gfMultiply(l[1], invDetS);
		} else if (syndromes.length == 6) {
			int detS = gfMultiply(gfMultiply(syndromes[0], syndromes[2]), syndromes[4]) ^ gfMultiply(gfMultiply(syndromes[2], syndromes[2]), syndromes[2]) ^ gfMultiply(gfMultiply(syndromes[1], syndromes[1]), syndromes[4]) ^ gfMultiply(gfMultiply(syndromes[3], syndromes[3]), syndromes[0]);
			if (detS == 0) {
				return computeErrorLocations(syndromes[0], syndromes[1], syndromes[2], syndromes[3]);// # matrix is non-invertible, throw away 2 syndromes
			}
			int invDetS = gfInvert(detS);
	...
	private static int computeSyndrome(int p, int j) {
		int s = 0;
		int n = 15;
		for (int k = n - 1; k >= 0; k--) {
			if ((p & 1) > 0) {
