package ru.r2cloud.jradio.beesat4;

import static org.junit.Assert.assertEquals;

import java.io.IOException;

import org.junit.Test;

import ru.r2cloud.jradio.fec.ViterbiTest;

public class Beesat4BeaconTest {

	@Test
	public void testSuccess() throws IOException {
		byte[] data = ViterbiTest.hexStringToByteArray(
				"");
		Beesat4Beacon beacon = new Beesat4Beacon();
		beacon.readFrameData(data);
		assertEquals(4, beacon.getFrames().size());
		TransferFrame f1 = beacon.getFrames().get(0);
		assertEquals("1acffc1d", f1.getASM());
		assertEquals(49461, f1.getFECFD());
		assertEquals(72, f1.getMCFC());
		assertEquals(0, f1.getPacket().getAPID());
		Apid0 apid0 = f1.getPacket().getApid0();
		assertEquals(5.275f, apid0.getACSC1X(), 0.0f);
		assertEquals(AcsMode.Suspend, apid0.getACSCMOD());
		Apid1 apid1 = beacon.getFrames().get(1).getPacket().getApid1();
		assertEquals(-13540, apid1.getACCM2X());
		assertEquals(8150, apid1.getACCM2Y());
		assertEquals(-490, apid1.getACCM2Z());
		Apid2 apid2 = beacon.getFrames().get(2).getPacket().getApid2();
		assertEquals(120.23926f, apid2.getCC0IN(), 0.0f);
		assertEquals(22.583008, apid2.getCC0OUT(), 0.00001f);
		Apid6_12 apid7 = beacon.getFrames().get(3).getPacket().getApid6_12();
		assertEquals(3757236279L, apid7.getCrc()[0]);
		assertEquals(1736542853, apid7.getCrc()[1]);
	}
}
