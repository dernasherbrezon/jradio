...
public class PhaseAmbiguityResolverTest {
	...
	public void test8Bit() {
		...
		Set<String> result = resolver.getSynchronizationMarkers();
		assertTrue(result.contains("00100111"));
		assertTrue(result.contains("01110010"));
		assertTrue(result.contains("11100100"));
		assertTrue(result.contains("00011011"));
		assertTrue(result.contains("10001101"));
		assertTrue(result.contains("10110001"));
		assertTrue(result.contains("11011000"));
		assertTrue(result.contains("01001110"));
	}

	@Test
	public void testSoftRotation8Bit() {
		PhaseAmbiguityResolver resolver = new PhaseAmbiguityResolver(0b00_10_01_11, 8);
		// no change
		byte[] rawData = ViterbiTest.hexStringToByteArray("0eb4");
		resolver.rotateSoft(rawData, 0b0010_0111);
		assertArrayEquals(ViterbiTest.hexStringToByteArray("0eb4"), rawData);

		rawData = ViterbiTest.hexStringToByteArray("0eb4");
		resolver.rotateSoft(rawData, 0b0111_0010);
		assertArrayEquals(ViterbiTest.hexStringToByteArray("0e4b"), rawData);

		rawData = ViterbiTest.hexStringToByteArray("0eb4");
		resolver.rotateSoft(rawData, 0b1110_0100);
		assertArrayEquals(ViterbiTest.hexStringToByteArray("4bf1"), rawData);

		rawData = ViterbiTest.hexStringToByteArray("0eb4");
		resolver.rotateSoft(rawData, 0b0001_1011);
		assertArrayEquals(ViterbiTest.hexStringToByteArray("b40e"), rawData);

		rawData = ViterbiTest.hexStringToByteArray("0eb4");
		resolver.rotateSoft(rawData, 0b1000_1101);
		assertArrayEquals(ViterbiTest.hexStringToByteArray("f1b4"), rawData);

		rawData = ViterbiTest.hexStringToByteArray("0eb4");
		resolver.rotateSoft(rawData, 0b1011_0001);
		assertArrayEquals(ViterbiTest.hexStringToByteArray("4B0E"), rawData);

		rawData = ViterbiTest.hexStringToByteArray("0eb4");
		resolver.rotateSoft(rawData, 0b1101_1000);
		assertArrayEquals(ViterbiTest.hexStringToByteArray("F14B"), rawData);

		rawData = ViterbiTest.hexStringToByteArray("0eb4");
		resolver.rotateSoft(rawData, 0b0100_1110);
		assertArrayEquals(ViterbiTest.hexStringToByteArray("B4F1"), rawData);
	}
